...
cm_score_file =  'scores/output.txt' 
# Replace ASV scores with organizers' scores or provide score file as the second argument.
asv_score_file = 'scores/asv_dev.txt' 
# asv_score_file = /home/chenchen/Data/SpeakerRecognition/ASVspoof/2019/LA/ASVspoof2019_LA_asv_scores/ASVspoof2019.LA.asv.eval.gi.trl.scores.txt
args = sys.argv
print("args = ",args)
if len(args) > 1:
    if len(args) != 3:
...
print("asv_data = ",asv_data)
asv_sources = asv_data[:, 0]    # 'bonafide' for genuine speech, or, the ID of the spoofing attack (A01 - A19)
asv_keys = asv_data[:, 1]       # 'target' for claimed speaker, or, 'nontarget' for impostor speaker, or, 'spoof' for spoofing speech
asv_scores = asv_data[:, 2].astype(np.float)    # similarity score value
